...
//Message should be messageToEncode@Key
// Main function to handle the encoding of message
char * encodeMessage(char * rawMessage){
  ...
  strncpy(encodedString, rawMessage, messageLen);
  ...
  for (i = 0; i < keyLength; i++){
      ...
      messageNum = ((int)encodedString[j] - 64);
      ...
      // Encode message
      keyNum = keyNum + messageNum;
      keyNum = keyNum % 27;
      encodedLetter = keyNum == 0 ? ' ' : (char)(keyNum + 64);

      // Update the string
      encodedString[j] = encodedLetter;
      j++;
  }
...
int runChildProcess(int * establishedConnectionFD){
  ...
  strcpy(encodedMessage, encodeMessage(message));

  charsRead = send(*establishedConnectionFD, encodedMessage, strlen(encodedMessage), 0); // Send success back
  if (charsRead < 0) error("ERROR writing to socket");

  // Client should now have the encrypted code
  _exit(0);
}
