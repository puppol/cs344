...
// Check for any illegal characters in their message text
void checkForBadChars(char * message){
	int len, i;
	len = strlen(message);

	for (i = 0; i < len-1; i++){
		if ( (message[i] < 'A' || message[i] > 'Z') && message[i] != ' ')
		error("Bad characters in the message.\n");
	}
}
...
int main(int argc, char *argv[]){
	...
	char buffer[70001];
	char message[140002];
  if (argc != 4) error("CLIENT: ERROR, Incorrect usage. Try: otp_enc message key port \n"); // Check usage & args
